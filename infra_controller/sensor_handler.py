...
class SensorHandler:
    ...
    def __init__(self):
        ...
        self.latest_ph = None  # NEW: pH storage
        self.rtd_mode = None
    ...
    def _handle_message(self, msg):
        ...
        if msg.get('type') == 'status':
            ...
            self.latest_ph = sensors.get('ph')  # NEW: Extract pH
            ...
            if self.messages_received % 10 == 1:
                # Updated log message to include pH
                if self.latest_ph is not None:
                    print(f"SENSOR: Temp: {self.latest_temp_c:.2f}°C / {self.latest_temp_f:.1f}°F, pH: {self.latest_ph:.3f}, Mode: {self.rtd_mode}")
                else:
    ...
    def get_ph(self):  # NEW: pH getter
        """Get latest pH reading"""
        if self.is_sensor_online():
            return self.latest_ph
        return None
    ...
    def get_status(self):
        ...
        return {
            ...
            "ph": self.latest_ph,  # NEW: Include pH in status
